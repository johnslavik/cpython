...
Module(
   body=[
    ...
    def test_get_docstring(self):
        ...
        node = ast.parse('type foo = int\n"""line one\n  line two"""', optimize=False)
        self.assertEqual(ast.get_docstring(node.body[0]),
                         'line one\nline two')
        self.assertEqual(ast.get_docstring(node.body[0], clean=False),
                         'line one\n  line two')

        node = ast.parse('x')
        self.assertRaises(TypeError, ast.get_docstring, node.body[0])

    def test_get_docstring_parenthesized(self):
        for form in ('("docstring")', '(("docstring"))', '("doc"\n  "string")'):
            with self.subTest(form=form):
                node = ast.parse(form, optimize=False)
                self.assertEqual(ast.get_docstring(node), 'docstring')

                node = ast.parse(f'class foo:\n  {form}', optimize=False)
                self.assertEqual(ast.get_docstring(node.body[0]), 'docstring')

                node = ast.parse(f'def foo():\n  {form}', optimize=False)
                self.assertEqual(ast.get_docstring(node.body[0]), 'docstring')

                node = ast.parse(f'async def foo():\n  {form}', optimize=False)
                self.assertEqual(ast.get_docstring(node.body[0]), 'docstring')

                node = ast.parse(f'type foo = int\n{form}', optimize=False)
                self.assertEqual(ast.get_docstring(node.body[0]), 'docstring')
